...
{phang}{bf:仅描述性统计：}省略 {opt depvar()}，并明确填写 {opt descvars()}。程序导出“描述性统计分析”RTF/DO/CSV 三件套、原始描述性统计、可选 DOCX、审计和日志，不创建 {cmd:_results.*} 或 {cmd:_main.ster}。{p_end}
...
{title:Main page: six empirical modules}
...
{pstd}每次运行生成带时间戳的 run ID，并写出：{p_end}
{p 8 12 2}1. {cmd:_descriptive.dta/.csv}；{p_end}
{p 8 12 2}2. {cmd:_results.dta/.csv}，包含 {cmd:specification_order} 和模型进入顺序的 {cmd:term_order}；{p_end}
{p 8 12 2}3. 可选 {cmd:_mediation.*}、{cmd:_heterogeneity.*}、{cmd:_parallel_trend.txt}、{cmd:_group_test.txt}、{cmd:_moderator_*.png} 和 {cmd:_report.docx}；{p_end}
{p 8 12 2}4. {cmd:_main.ster}、完整 {cmd:.log} 和 {cmd:_audit.txt}。{p_end}
...
{pstd}所有临时缩尾、重分类和图形估计都在 {cmd:preserve}/{cmd:restore} 副本中运行。审计文件记录运行前后数据签名。{p_end}
...
{p 8 12 2}{cmd:. net install journalone, from("https://your-host/path/journalone") replace}{p_end}
...
{pstd}该命令安全地向个人 {cmd:profile.do} 追加菜单块，不会清空既有用户菜单，并加入“一键更新”入口。详细数据清洗说明见 {help journalone_prep}。{p_end}
