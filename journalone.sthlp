...
{pstd}每个 RTF 表格下方只追加一段简洁的重点解读，不使用“结果解读”“合理性检查”等程序化小标题。描述性统计保留中文表题、表注和重点解读，仅将统计指标列名输出为英文（{cmd:Variable}、{cmd:N}、{cmd:Mean}、{cmd:SD}、{cmd:Min}、{cmd:Max}）；变量名仍保持 Stata 原始名称。回归表和诊断表只突出核心系数、P 值、方向、显著性、跨规格稳定性及关键诊断风险。自动文字只读取本次实际结果，不修改数据、模型或显著性，也不会把不显著结果解释为显著。{p_end}
